    ...
    override fun onCommand(
        ...
        if (player == null)
        {
            sender.spigot().sendMessage(
                *ComponentBuilder("\uD83D\uDC64").color(ChatColor.RED)
                    ...
                    .append(
                        SecurityPL3.getTranslation("player_not_found",
        ...
        sender.spigot().sendMessage(
            *ComponentBuilder("\uD83D\uDC64").color(ChatColor.GREEN)
                .append(" » ").color(ChatColor.GRAY)
                .append(SecurityPL3.getTranslation("admin_forced_change_password",
                    if (sender is Player) sender.locale else "default"
                )).reset()
                .create()
        )
        player.spigot().sendMessage(
            *ComponentBuilder("\uD83D\uDD11").color(ChatColor.BLUE)
                .append(" » ").color(ChatColor.GRAY)
                .append(SecurityPL3.getTranslation("player_forced_change_password", player.locale)).reset()
                .create()
        )
